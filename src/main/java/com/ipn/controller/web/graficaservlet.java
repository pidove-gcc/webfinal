...
public class graficaservlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(accion.equals("reportegrafica")){
            reporteGrafica(request, response);
        }else{
        }
    }
    ...
    private void reporteGrafica(HttpServletRequest request, HttpServletResponse response) throws SQLException, JRException {
        ArticuloDAO dao = new ArticuloDAO();
        try {
            try (ServletOutputStream sos = response.getOutputStream()) {
                File reporte = new File(getServletConfig().getServletContext().getRealPath("/grafica/Grafica.jasper"));
                byte[] bytes;
